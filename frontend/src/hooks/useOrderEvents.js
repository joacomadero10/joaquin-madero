 ...
 * "nuevo_pedido" / "pedido_actualizado" / "mesa_actualizada". Pensado para
 * las pantallas de Cocina, Mozo y el seguimiento de pedido del Cliente.
 *
 ...
 */
export function useOrderEvents(restaurantId, { onNuevoPedido, onPedidoActualizado, onMesaActualizada } = {}) {
  const callbacksRef = useRef({ onNuevoPedido, onPedidoActualizado, onMesaActualizada });
  callbacksRef.current = { onNuevoPedido, onPedidoActualizado, onMesaActualizada };
  ...
  useEffect(() => {
    ...
    socket.on('mesa_actualizada', (table) => {
      callbacksRef.current.onMesaActualizada?.(table);
    });
